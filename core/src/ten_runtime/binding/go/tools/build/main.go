...
//
// The final application executable will link ten_runtime_go (static library),
// if asan is enabled in compiling ten_runtime_go, the C compiler must be same as the
// compiler used to compile ten_runtime_go.
//
...
// findMinGWGcc() searches for MinGW gcc in PATH environment variable.
// It looks for directories containing "mingw" and ending with "bin",
...
func findMinGWGcc() string {
	...
	for _, pathDir := range strings.Split(pathEnv, string(os.PathListSeparator)) {
		...
		pathDirLower := strings.ToLower(pathDir)
		if !strings.Contains(pathDirLower, "mingw") || !strings.HasSuffix(pathDirLower, "bin") {
			continue
		}

		// Verify it's a real MinGW directory by checking for gcc.exe
		gccPath := path.Join(pathDir, "gcc.exe")
		if IsFilePresent(gccPath) {
			return gccPath
		}
	}
...
func (ab *AppBuilder) addRuntimeLdflags() {
	...
	case "linux", "darwin", "windows":
		...
		if runtime.GOOS == "windows" && cc != "gcc" {
			// CGO doesn't work with MSVC
			// .c files generated by CGO contain GCC syntax that MSVC doesn't understand.
			// (Such as 'attribute((unused))', '#pragma GCC diagnostic')
			// Ref: https://github.com/golang/go/issues/20982
			//		https://github.com/golang/go/issues/74189
			mingwGcc := findMinGWGcc()
			if mingwGcc != "" {
				log.Printf("Detected non-gcc compiler (%s), switching to MinGW gcc: %s", cc, mingwGcc)
				ab.cachedEnv["CC"] = mingwGcc
			} else {
				log.Printf("Unsupported compiler %s for platform %s, consider installing MinGW.", cc, runtime.GOOS)
				log.Fatalf("Note that we will auto-detect and force use of MinGW compiler for GO.")
			}
		}
		...
		if runtime.GOOS == "windows" {
			...
			// Linux/macOS: Link against shared libraries
