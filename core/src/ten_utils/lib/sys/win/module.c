...
void *ten_module_load(const ten_string_t *name, int as_local) {
  ...
  // to find ten_runtime.dll, ten_utils.dll and ten_runtime_python.dll.
  ...
  //    ten_utils.dll, ten_runtime_python.dll) are NOT in these default search
  //    paths, we must explicitly add their directories using AddDllDirectory.
  ...
  // searched.
  TEN_LOGI("Calling LoadLibraryExA for '%s'", dll_path);
