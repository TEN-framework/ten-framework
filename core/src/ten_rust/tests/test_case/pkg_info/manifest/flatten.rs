...
mod tests {
    ...
    #[tokio::test]
    async fn test_manifest_flatten_with_file_url() {
        // Create a temporary directory for testing
        let temp_dir = TempDir::new().unwrap();
        let temp_path = temp_dir.path();
        ...
        let readme_content = "This is a test README content.";
        let readme_file = temp_path.join("readme.md");
        fs::write(&readme_file, readme_content).unwrap();

        // Create a manifest JSON with file:// URL import_uri
        let manifest_json = format!(
            r#"{{
            "type": "extension",
            "name": "test_extension",
    ...
    #[tokio::test]
    async fn test_manifest_flatten_with_relative_path() {
        // Create a temporary directory for testing
        let temp_dir = TempDir::new().unwrap();
        let temp_path = temp_dir.path();
    ...
    #[tokio::test]
    async fn test_manifest_flatten_with_multiple_fields() {
        // Create a temporary directory for testing
        let temp_dir = TempDir::new().unwrap();
        let temp_path = temp_dir.path();

        // Create test content files
        let readme_content = "README content";
        let description_content = "Description content";
        let display_name_content = "Display Name";
        ...
        // Create a manifest JSON with multiple import_uri fields
        let manifest_json = r#"{
            "type": "extension",
            "name": "test_extension",
            "version": "1.0.0",
    ...
    #[tokio::test]
    async fn test_manifest_flatten_with_existing_content_preserved() {
        // Create a temporary directory for testing
        let temp_dir = TempDir::new().unwrap();
        let temp_path = temp_dir.path();

        // Create a test file (this should not be used)
        fs::write(
            temp_path.join("readme.md"),
            "File content should not be used",
        )
        .unwrap();

        // Create a manifest JSON with existing content (should be preserved)
        let existing_content = "Existing content should be preserved";
        let manifest_json = format!(
            r#"{{
            "type": "extension",
            "name": "test_extension",
    ...
    #[tokio::test]
    async fn test_manifest_flatten_missing_file_error() {
        // Create a manifest JSON with import_uri pointing to non-existent file
        let manifest_json = r#"{
            "type": "extension",
            "name": "test_extension",
