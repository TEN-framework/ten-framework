...
mod tests {
    ...
    async fn test_populate_selector_message_info() {
        use ten_rust::graph::Graph;
        use ten_rust::graph::node::SelectorMessageInfo;
        use ten_rust::pkg_info::message::{MsgDirection, MsgType};

        let graph_str = include_str!("../../test_data/graph_with_selector/graph_for_populate_selector_message_info.json");
        ...
        let selector_1_node = graph.nodes.iter().find(|node| {
            if let ten_rust::graph::node::GraphNode::Selector { content } = node {
        ...
        if let ten_rust::graph::node::GraphNode::Selector { content } = selector_1_node {
            ...
            for msg in &content.messages {
                assert_eq!(msg.direction, MsgDirection::Out,
                    "All messages should be Out direction");
            }

            // Check specific messages exist
            assert!(content.messages.iter().any(|m|
                m.msg_type == MsgType::Cmd && m.msg_name == "command_a" && m.direction == MsgDirection::Out
            ), "Missing command_a");

            assert!(content.messages.iter().any(|m|
                m.msg_type == MsgType::Cmd && m.msg_name == "command_b" && m.direction == MsgDirection::Out
            ), "Missing command_b");

            assert!(content.messages.iter().any(|m|
                m.msg_type == MsgType::Data && m.msg_name == "data_x" && m.direction == MsgDirection::Out
            ), "Missing data_x");

            assert!(content.messages.iter().any(|m|
                m.msg_type == MsgType::Data && m.msg_name == "data_y" && m.direction == MsgDirection::Out
            ), "Missing data_y");

            assert!(content.messages.iter().any(|m|
                m.msg_type == MsgType::AudioFrame && m.msg_name == "audio_stream_1" && m.direction == MsgDirection::Out
        ...
        let selector_all_node = graph.nodes.iter().find(|node| {
            if let ten_rust::graph::node::GraphNode::Selector { content } = node {
        ...
        if let ten_rust::graph::node::GraphNode::Selector { content } = selector_all_node {
            ...
            let cmd_messages: Vec<&SelectorMessageInfo> = content.messages.iter()
                .filter(|m| m.msg_type == MsgType::Cmd)
                .collect();
            assert_eq!(cmd_messages.len(), 2, "Expected 2 cmd messages");

            let data_messages: Vec<&SelectorMessageInfo> = content.messages.iter()
                .filter(|m| m.msg_type == MsgType::Data)
                .collect();
            assert_eq!(data_messages.len(), 3, "Expected 3 data messages");

            let audio_messages: Vec<&SelectorMessageInfo> = content.messages.iter()
                .filter(|m| m.msg_type == MsgType::AudioFrame)
                .collect();
            assert_eq!(audio_messages.len(), 1, "Expected 1 audio message");

            let video_messages: Vec<&SelectorMessageInfo> = content.messages.iter()
                .filter(|m| m.msg_type == MsgType::VideoFrame)
                .collect();
            assert_eq!(video_messages.len(), 2, "Expected 2 video messages");

            // Verify specific message names
            let message_names: Vec<String> = content.messages.iter()
                .map(|m| m.msg_name.clone())
                .collect();
            ...
            assert!(message_names.contains(&"audio_stream_1".to_string()), "Missing audio_stream_1");
            assert!(message_names.contains(&"video_stream_1".to_string()), "Missing video_stream_1");
            assert!(message_names.contains(&"video_stream_2".to_string()), "Missing video_stream_2");
        } else {
            panic!("Expected selector node");
        }
    }
