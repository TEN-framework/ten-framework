...
mod tests {
    ...
    //TODO: implement other_fields instead of using all_fields
    //#[actix_web::test]
    ...
    //     assert_eq!(property.get("key1").unwrap().as_str().unwrap(),
    // "value1");     assert_eq!(property.get("key2").unwrap().as_i64().
    // unwrap(), 42);     assert!(property.get("key3").unwrap().
    // is_object()); }
