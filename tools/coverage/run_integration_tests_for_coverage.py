...
def main() -> int:
	...
	if is_clang:
		...
		env["LLVM_PROFILE_FILE"] = os.path.join(prof_dir, "profile-int-%p-%m.profraw")

	# Make out/<os>/<cpu>/tests importable as a package root for implicit namespace packages
	tests_root: str = os.path.join(root_out, "tests")
	env["PYTHONPATH"] = tests_root + (os.pathsep + env["PYTHONPATH"] if "PYTHONPATH" in env and env["PYTHONPATH"] else "")
	...
	for root, _dirs, files in os.walk(integ_root):
		...
		_skip = (not numpy_available) and ("/ffmpeg_" in root or "/ffmpeg/" in root)
		if _skip:
