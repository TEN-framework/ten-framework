...
class AzureASRExtension(AsyncASRBaseExtension):
    ...
    def _cancel_transport_recovery(self) -> None:
        if (
            self._transport_recovery_task is not None
            and not self._transport_recovery_task.done()
        ):
            self._transport_recovery_task.cancel()
    ...
    async def _transport_recovery_after_grace(
        ...
        if self.stopped or disconnect_epoch != self._transport_disconnect_epoch:
            return
