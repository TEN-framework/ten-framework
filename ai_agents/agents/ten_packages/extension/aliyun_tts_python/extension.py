...
from .aliyun_tts import (
    AliyunTTSPython,
    AliyunTTSTaskFailedException,
...
class AliyunTTSPythonExtension(AsyncTTS2BaseExtension):
    def __init__(self, name: str) -> None:
        super().__init__(name)
        self.config: AliyunTTSPythonConfig | None = None
        self.client: AliyunTTSPython | None = None
    ...
    async def on_init(self, ten_env: AsyncTenEnv) -> None:
        try:
            ...
            if self.config is None:
                ...
                self.config = AliyunTTSPythonConfig.model_validate_json(
                    config_json
                )
            ...
            ten_env.log_info(
                "AliyunTTSPython client initialized and preheated successfully"
            )
    ...
    async def request_tts(self, t: TTSTextInput) -> None:
        ...
        try:
                ...
                if self.config and self.config.dump:
                    ...
                    if t.request_id not in self.recorder_map:
                        dump_file_path = os.path.join(
                            ...
                            f"aliyun_dump_{t.request_id}.pcm",
                        )
            ...
            self.ten_env.log_error(
                f"AliyunTTSTaskFailedException in request_tts: {e.error_msg} (code: {e.error_code}). text: {t.text}"
            )
    ...
    async def _handle_audio_data(
        ...
        try:
            ...
            if event_status == EVENT_TTSResponse:
                ...
                self.ten_env.log_info(
                    "Received TTSSentenceEnd event from Aliyun TTS"
                )
                ...
                if (
                    ...
                    # await self.send_char_audio_metrics(self.current_request_id)
                    # 重置状态为下一个请求做准备
                ...
                self.ten_env.log_info(
                    f"KEYPOINT Received task finished event from Aliyun TTS for request ID: {self.current_request_id}"
                )
    ...
    async def _send_tts_error_for_exception(
        self, exception: AliyunTTSTaskFailedException
    ) -> None:
        """统一的TTS异常错误发送方法"""
