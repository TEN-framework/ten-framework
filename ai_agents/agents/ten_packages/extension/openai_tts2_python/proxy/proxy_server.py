    ...
    try:
        ...
        if "authorization" in headers:
            ...
            if len(auth_header) > 20:
                masked_auth = auth_header[:10] + "***MASKED***" + auth_header[-10:]
            else:
                masked_auth = "***MASKED***"
        ...
        try:
            ...
            for key, value in response.headers.items():
                ...
                if key_lower not in headers_to_exclude:
                    if key_lower in ["content-type"] or key_lower.startswith("openai-") or key_lower.startswith("x-"):
                        essential_headers[key] = value
            
            if "content-type" not in essential_headers:
                essential_headers["content-type"] = response.headers.get("content-type", "audio/pcm")
            ...
            async def generate():
                ...
                try:
                    ...
                    async for raw_chunk in response.aiter_raw():
                        if raw_chunk:
                            ...
                            if chunk_count <= 5 or chunk_count % 10 == 0:
                                logger.info(f"  Chunk #{chunk_count}: {len(raw_chunk)} bytes (total: {total_bytes} bytes)")
                            yield raw_chunk
                    logger.info(f"Streaming complete: {chunk_count} chunks, {total_bytes} total bytes")
                except Exception as e:
                    error_msg = str(e)
                    error_type = type(e).__name__
                    logger.warning(f"Streaming interrupted: {error_type}: {error_msg}")
                    logger.warning(f"  Streamed {chunk_count} chunks, {total_bytes} bytes before interruption")
                    ...
                    if (
                        ...
                        logger.debug("Client disconnected early, stopping stream gracefully")
                    return
                finally:
            ...
            logger.info(f"Returning StreamingResponse with headers: {essential_headers}")
            logger.info("=" * 80)
        ...
        logger.error(f"HTTP status error while proxying: {e.response.status_code} - {e}")
        return Response(
