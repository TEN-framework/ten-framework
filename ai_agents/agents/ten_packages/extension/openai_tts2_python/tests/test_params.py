...
@patch("openai_tts2_python.openai_tts.Timeout")
@patch("openai_tts2_python.openai_tts.Limits")
def test_enable_context_payload(MockLimits, MockTimeout, MockAsyncClient):
    """Tests that context fields are only added when enable_context is true."""
    from openai_tts2_python.openai_tts import OpenAITTSClient
    from openai_tts2_python.config import OpenAITTSConfig
    from ten_runtime import AsyncTenEnv
    from unittest.mock import MagicMock
...
@patch("openai_tts2_python.openai_tts.Timeout")
@patch("openai_tts2_python.openai_tts.Limits")
def test_request_tts_passes_context_markers(
    MockLimits, MockTimeout, MockAsyncClient
):
    """Tests that the extension forwards first/last chunk markers to the client."""
    ...
    extension = OpenAITTSExtension("tts")
    extension.ten_env = MagicMock()
    extension.ten_env.log_info = MagicMock()
    extension.ten_env.log_debug = MagicMock()
    ...
    assert first_history[0]["content"][0]["text"] == "old user"
    assert first_history[1]["role"] == "assistant"
