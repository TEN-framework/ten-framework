...
def test_dump_functionality(MockStepFunTTSWebsocket):
    ...
    def capture_callback(*args, **kwargs):
        ...
        nonlocal audio_callback
        if "on_audio_data" in kwargs:
            audio_callback = kwargs["on_audio_data"]
        return mock_instance

    MockStepFunTTSWebsocket.side_effect = capture_callback

    # Mock the callback-based audio handling for StepFun TTS
    async def mock_get(tts_input):
        ...
        if audio_callback:
            ...
            await audio_callback(b"", 350, 0)  # EVENT_TTSSentenceStart
            await asyncio.sleep(0.01)

            # Simulate audio chunks
            await audio_callback(fake_audio_chunk_1, EVENT_TTSResponse, 0)
            await asyncio.sleep(0.01)
            await audio_callback(fake_audio_chunk_2, EVENT_TTSResponse, 100)
            await asyncio.sleep(0.01)
...
@patch("stepfun_tts_python.extension.StepFunTTSWebsocket")
def test_flush_logic(MockStepFunTTSWebsocket):
    """
    ...
    # Store the callback that will be set by the extension
    flush_audio_callback = None

    def capture_flush_callback(*args, **kwargs):
        # Capture the on_audio_data callback from the keyword arguments
        nonlocal flush_audio_callback
        if "on_audio_data" in kwargs:
            flush_audio_callback = kwargs["on_audio_data"]
        return mock_instance

    MockStepFunTTSWebsocket.side_effect = capture_flush_callback

    async def mock_get_long_audio_stream(tts_input):
        # Wait a bit to ensure callback is captured
        await asyncio.sleep(0.01)

        if flush_audio_callback:
            for i in range(20):
                if mock_instance.cancel.called:
                    print(
                        "Mock detected cancel call, stopping stream and yielding EVENT_TTSFlush."
                    )
                    await flush_audio_callback(b"", EVENT_TTSFlush, 0)
                    return  # Stop the generator immediately

                # Simulate audio chunks
                await flush_audio_callback(
                    b"\x11\x22\x33" * 100, EVENT_TTSResponse, i * 100
                )
