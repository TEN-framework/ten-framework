...
def test_invalid_api_key_error(MockAsyncClient):
    ...
    mock_client.post.side_effect = Exception(
        "Client error '401 Unauthorized' for url 'https://matcha.ezai-k8s.freeddns.org/tts'"
    )
    ...
    assert (
        vendor_info.get("vendor") == "ezai"
    ), f"Expected vendor 'ezai', got {vendor_info.get('vendor')}"
