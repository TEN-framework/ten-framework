...
def test_sequential_requests_state_machine(MockElevenLabsTTS2Client):
        ...
        async def populate_queue():
            ...
            if len(request_order) == 0:
                ...
                await mock_instance.response_msgs.put(
                    (b"mock_audio_data_1", False, "", 123)
                )
                await asyncio.sleep(0.01)
                await mock_instance.response_msgs.put(
                    (b"mock_audio_data_2", False, "", None)
                )
                await asyncio.sleep(0.01)
                await mock_instance.response_msgs.put(
                    (b"mock_audio_data_3", True, "First request text", None)
                )
                await asyncio.sleep(0.01)
                # Second request audio chunks
                request_order.append("request_2")
                print(f"  → Mock: Starting synthesis for request 2")
                await mock_instance.response_msgs.put(
                    (b"mock_audio_data_4", False, "", 123)
                )
                await asyncio.sleep(0.01)
                await mock_instance.response_msgs.put(
                    (b"mock_audio_data_5", False, "", None)
                )
                await asyncio.sleep(0.01)
                await mock_instance.response_msgs.put(
                    (b"mock_audio_data_6", True, "Second request text", None)
                )
    ...
    # Set up other required mock methods
...
def test_request_state_transitions(MockElevenLabsTTS2Client):
    ...
    def mock_client_init(
        ...
        async def populate_queue():
            await asyncio.sleep(0.01)
            await mock_instance.response_msgs.put(
                (b"audio_chunk", False, "", 123)
            )
            await asyncio.sleep(0.01)
            await mock_instance.response_msgs.put(
                (b"", True, "Test state transitions", None)
            )
