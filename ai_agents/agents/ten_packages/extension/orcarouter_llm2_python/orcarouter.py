...
@dataclass
class OrcaRouterLLM2Config(BaseModel):
    api_key: str = ""
    base_url: str = "https://api.orcarouter.ai/v1"
    # `orcarouter/auto` is OrcaRouter's per-account router: it picks a live
    # model for each request (strategy is configured in the OrcaRouter
    # console). Any provider-prefixed model ID also works, e.g.
    # "openai/gpt-4o" or "orcarouter/fusion".
...
class OrcaRouterLLM:
    ...
    def __init__(self, ten_env: AsyncTenEnv, config: OrcaRouterLLM2Config):
        self.config = config
        self.ten_env = ten_env
        ten_env.log_info(
            f"OrcaRouterLLM initialized with model: {config.model} "
        ...
        # OrcaRouter is OpenAI-compatible and authenticates with a Bearer
        # token. `custom_headers` lets callers add attribution headers
        # (HTTP-Referer / X-Title) so the OrcaRouter console can report which
        # client is calling; a caller-supplied value always wins.
        default_headers = {
    ...
    async def get_chat_completions(
        ...
        # Build request. OrcaRouter normalizes provider-specific parameter
        # constraints server-side (e.g. sampling params such as `temperature`
        # are accepted even when a request is routed to a reasoning model), so
        # we always send the standard OpenAI chat-completion parameters.
        req = {
        ...
        # This is also how OrcaRouter routing controls are passed, e.g.
        # parameters={"models": ["openai/gpt-4o", "anthropic/claude-haiku-4.5"],
        # "route": "fallback"} enables a model-fallback chain.
        for key, value in (request_input.parameters or {}).items():
            ...
            if not self.config.is_black_list_params(key):
                self.ten_env.log_debug(f"set orcarouter param: {key} = {value}")
                req[key] = value
        ...
        try:
            ...
            if tool_calls_list:
                for tool_call in tool_calls_list:
                    ...
                    self.ten_env.log_info(
                        f"Tool call: {choice.delta.model_dump_json()}"
                    )
