...
class OpenAILLM2Extension(AsyncLLM2BaseExtension):
    ...
    async def on_update_prompt(self, async_ten_env: AsyncTenEnv, request_id: str, prompt: str) -> None:
        self.config.prompt = prompt
        async_ten_env.log_info(f"Updated prompt for request_id: {request_id}")

    async def on_retrieve_prompt(self, async_ten_env: AsyncTenEnv, request: LLMRequestRetrievePrompt) -> LLMResponseRetrievePrompt:
