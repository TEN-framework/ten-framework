...
class AgoraGenericRecorder:
    ...
    def __init__(self, app_id: str, app_cert: str, api_key: str, channel_name: str, avatar_uid: int, ten_env: AsyncTenEnv, avatar_id: str, quality: str, version: str, video_encoding: str, enable_string_uid: bool, start_endpoint: str, stop_endpoint: str, activity_idle_timeout: int):
    ...
    async def disconnect(self):
        ...
        if self.heartbeat_task:
            ...
            try:
                ...
                self.ten_env.log_error(f"Error while cancelling heartbeat task: {e}")
        ...
        if self.websocket_task:
            ...
            try:
                ...
                self.ten_env.log_error(f"Error while cancelling WebSocket task: {e}")
    ...
    async def _raise_for_status_verbose(self, response):
        try:
            ...
            try:
                if response.headers.get('content-type', '').startswith('application/json'):
                    ...
                    if 'message' in error_data:
                        error_details += f". Server message: {error_data['message']}"
            ...
            await self._handle_error(error_details, code=ERROR_CODE_REQUEST_HTTP_ERROR)
            raise
    ...
    async def _stop_session(self, session_id: str):
        try:
            ...
            self.ten_env.log_info(f"Headers: {json.dumps({k: v for k, v in headers.items() if k != 'authorization'}, indent=2)}")
            self.ten_env.log_info(f"Authorization: Bearer ***masked***")
            self.ten_env.log_info(f"Payload: {json.dumps(payload, indent=2)}")

            # Use DELETE method as specified in API documentation
            response = requests.delete(self.stop_endpoint, json=payload, headers=headers)
            await self._raise_for_status_verbose(response)
            self._clear_session_id_cache()
        except Exception as e:
    ...
    async def _connect_websocket_loop(self):
        ...
        while self._should_reconnect:
            try:
                ...
                self.ten_env.log_info(f"Connecting to WebSocket at {self.realtime_endpoint} (attempt {attempt + 1})")
    ...
    async def _handle_connection_error(self, error: Exception, attempt: int) -> None:
        ...
        if "Failed to connect to media server" in str(error) or "Connection refused" in str(error):
    ...
    async def _debounced_speak_end(self):
        while True:
            try:
                await asyncio.wait_for(self._speak_end_event.wait(), timeout=SPEAK_END_TIMEOUT)
    ...
    async def _send_message(self, message: dict) -> bool:
        ...
        try:
            ...
            self.ten_env.log_debug(f"Sent message: {message.get('command', 'unknown')}")
            return True
        except Exception as e:
