...
def patch_soniox_ws():
    ...
    with patch(patch_target) as MockWebsocketClient:
        ...
        async def trigger_transcript(tokens, final_audio_proc_ms, total_audio_proc_ms):
            if 'transcript' in websocket_client_instance._callbacks:
                await websocket_client_instance._callbacks['transcript'](tokens, final_audio_proc_ms, total_audio_proc_ms)
        
        async def trigger_error(error_code, error_message):
            if 'error' in websocket_client_instance._callbacks:
                await websocket_client_instance._callbacks['error'](error_code, error_message)
        
        async def trigger_exception(exception):
            if 'exception' in websocket_client_instance._callbacks:
                await websocket_client_instance._callbacks['exception'](exception)
        
        async def trigger_finished(final_audio_proc_ms, total_audio_proc_ms):
            if 'finished' in websocket_client_instance._callbacks:
                await websocket_client_instance._callbacks['finished'](final_audio_proc_ms, total_audio_proc_ms)
