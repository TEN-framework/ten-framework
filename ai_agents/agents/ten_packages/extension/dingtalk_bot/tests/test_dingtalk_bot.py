...
class TestDingTalkBot(ExtensionTester):
    ...
    def result_handler(self, ten_env: TenEnvTester, result: CmdResult | None, error: TenError | None):
        """
        This method is the callback for send_cmd.
        It receives the result of the command execution.
        """
        print(f"[TEST_RUNNER] Result handler called. Received result: {result}, error: {error}")
...
def test_send_message_via_tester():
    ...
    # Set up the tester to run the 'dingtalk_bot' extension
    tester.set_test_mode_single("dingtalk_bot", property_json_content)
    ...
    assert completed, "Test timed out, did not receive a result from the extension."

    print("[PYTEST] Asserting results...")
    # Assertions on the result
    assert tester.test_error is None, f"Command execution returned an error: {tester.test_error}"
