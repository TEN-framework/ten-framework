...
def test_empty_params_fatal_error():
    ...
    tester.set_test_mode_single("xai_tts_python", json.dumps(empty_params_config))
...
@patch("xai_tts_python.xai_tts.websockets.connect")
def test_invalid_api_key_error(mock_websocket_connect):
    ...
    tester.set_test_mode_single("xai_tts_python", json.dumps(invalid_key_config))
    ...
    assert (
        vendor_info.get("vendor") == "xai"
    ), f"Expected vendor 'xai', got {vendor_info.get('vendor')}"
