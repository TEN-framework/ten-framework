...
def test_sequential_requests(MockClient):
    ...
    tester.set_test_mode_single("xai_tts_python", json.dumps(MOCK_CONFIG))
    tester.run()
...
def test_connect_backoff_limit():
    async def _run():
        ten_env = MagicMock()
        ...
        with patch.object(
            ...
            try:
                ...
                raise AssertionError("Expected reconnect ceiling failure")

        assert client._connect_exp_cnt == 5
