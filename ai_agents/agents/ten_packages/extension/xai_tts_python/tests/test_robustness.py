...
def test_empty_text(MockXAITTSClient):
...
@patch("xai_tts_python.extension.XAITTSClient")
def test_whitespace_text(MockXAITTSClient):
...
class ExtensionTesterPunctuationText(ExtensionTester):
    def __init__(self):
        super().__init__()
...
@patch("xai_tts_python.extension.XAITTSClient")
def test_punctuation_only_text(MockXAITTSClient):
    MockXAITTSClient.return_value = create_mock_client()

    tester = ExtensionTesterPunctuationText()
    tester.set_test_mode_single(
        "xai_tts_python",
        json.dumps(
            {
    ...
    assert tester.error_received, "Punctuation-only text should raise an error."
...
@patch("xai_tts_python.extension.XAITTSClient")
def test_long_text(MockXAITTSClient):
...
@patch("xai_tts_python.extension.XAITTSClient")
def test_special_characters(MockXAITTSClient):
