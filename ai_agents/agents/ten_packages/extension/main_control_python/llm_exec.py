...
class LLMExec:
    ...
    def __init__(self, ten_env: AsyncTenEnv):
        ...
        self.stopped = False
        self.on_response: Optional[Callable[[AsyncTenEnv, str, bool], Awaitable[None]]] = None
    ...
    async def _process_input_queue(self):
        ...
        while not self.stopped:
            try:
                ...
                self.current_task = self.loop.create_task(self._send_to_llm(self.ten_env, item, is_final=False))
                await self.current_task
            except asyncio.CancelledError:
                self.ten_env.log_info("LLMExec processing cancelled.")
            except Exception as e:
                self.ten_env.log_error(f"Error processing input queue: {traceback.format_exc()}")
            finally:
                self.current_task = None


    async def _send_to_llm(self, ten_env: AsyncTenEnv, text: str, is_final: bool = None) -> None:
        response = _send_cmd_ex(ten_env, "chat_completion", "llm", {
