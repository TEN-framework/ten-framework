...
class TwilioServer:
    ...
    def _setup_twilio_client(self):
        ...
        if self.config.twilio_account_sid and self.config.twilio_auth_token:
            try:
                ...
                if self.ten_env:
                    self.ten_env.log_info("Twilio client initialized successfully")
            except Exception as e:
                if self.ten_env:
                    self.ten_env.log_error(f"Failed to initialize Twilio client: {e}")
    ...
    def _setup_routes(self):
        ...
        async def create_call(request: Request):
            ...
            try:
                ...
                if not self.config.twilio_webhook_url:
                ...
                twiml_response.start().stream(url=f"wss://{self.config.twilio_webhook_url}:{self.config.websocket_port}/ws")
                twiml_response.pause(length=30)

                # Add status callback for call events
                status_callback_url = f"https://{self.config.twilio_webhook_url}:{self.config.http_port}/webhook/status"
                ...
                if self.ten_env:
                    self.ten_env.log_info(f"Outbound call initiated: SID={call.sid}, To={phone_number}")
        ...
        async def delete_call(call_sid: str):
            ...
            try:
                ...
                return JSONResponse(content={"message": f"Call {call_sid} stopped"})
        ...
        async def twilio_status_webhook(request: Request):
            ...
            try:
                ...
                if self.ten_env:
                    self.ten_env.log_info(f"Received status webhook: CallSid={call_sid}, Status={call_status}")
                ...
                if call_sid and call_sid in self.active_call_sessions:
                    ...
                    if self.ten_env:
                        self.ten_env.log_info(f"Call {call_sid} status updated to: {call_status}")

                    # Clean up completed calls
                    if call_status in ["completed", "failed", "busy", "no-answer", "canceled"]:
                        asyncio.create_task(self._cleanup_call_after_delay(call_sid, 10))
                ...
                if self.ten_env:
                    self.ten_env.log_error(f"Error processing status webhook: {e}")
        ...
        async def websocket_endpoint(websocket: WebSocket):
            ...
            try:
                ...
                if call_sid in self.active_call_sessions:
                    ...
                    if self.ten_env:
                        self.ten_env.log_warn(f"Call SID {call_sid} not found in active sessions.")
    ...
    async def start_websocket_server(self):
        ...
        async def websocket_handler(websocket, path):
            await self.app.websocket("/ws")(websocket)
        ...
        if self.ten_env:
            self.ten_env.log_info(f"WebSocket server started on port {self.config.websocket_port}")
