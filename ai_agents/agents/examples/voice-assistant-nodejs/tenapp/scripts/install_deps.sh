...
install_python_requirements() {
  ...
  if [[ -d "$app_dir/ten_packages/extension" ]]; then
    echo "Traversing ten_packages/extension directory for Python dependencies..."
    for extension in "$app_dir/ten_packages/extension"/*; do
      if [[ -d "$extension" && -f "$extension/requirements.txt" ]]; then
        echo "Found requirements.txt in $extension, installing Python dependencies..."
  ...
  if [[ -d "$app_dir/ten_packages/system" ]]; then
    echo "Traversing ten_packages/system directory for Python dependencies..."
...
main() {
  ...
  echo "Using npm command: $NPM_INSTALL_CMD"
  ...
  echo "All dependencies installation completed successfully!"
}
