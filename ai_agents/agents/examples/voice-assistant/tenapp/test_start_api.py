...
def simulate_start_api(graph_name, channel_override):
    ...
    if agora_node:
        ...
        print(f"    ✓ Property override supported (uses standard 'channel' property)")
    ...
    if avatar_node:
        ...
        if addon == 'heygen_avatar_python':
            ...
            required_props = ['heygen_api_key', 'agora_appid', 'agora_avatar_uid', 'input_audio_sample_rate']
            ...
            current_channel = avatar_node['property'].get('agora_channel_name', 'unknown')
            print(f"    - Current agora_channel_name property: {current_channel}")
            print(f"    ⚠ WARNING: Uses 'agora_channel_name' (may need backend update for override)")

            # Check other properties
            required_props = [
                'generic_video_api_key', 'agora_appid', 'agora_video_uid',
                'avatar_id', 'quality', 'version', 'video_encoding',
                'start_endpoint', 'stop_endpoint', 'input_audio_sample_rate'
            ]
            for prop in required_props:
                if prop in avatar_node['property']:
                    value = avatar_node['property'][prop]
    ...
    tts_conn = next((c for c in connections if c.get('extension') == 'tts'), None)
    ...
    main_control_conn = next((c for c in connections if c.get('extension') == 'main_control'), None)
...
def test_error_scenarios():
    ...
    graphs = config['ten']['predefined_graphs']
    invalid_graph = next((g for g in graphs if g["name"] == "invalid_graph_name"), None)
...
if __name__ == "__main__":
    ...
    results.append((
        "voice_assistant",
        simulate_start_api("voice_assistant", test_channel)
    ))
