...
class AppendInputWithoutTextInputEndTester(AsyncExtensionTester):
    ...
    def __init__(
        ...
        print("=" * 80)
        print("📋 Test Description: Validate TTS append input with multiple text inputs without text input end")
        print("🎯 Test Objectives:")
        print("   - Verify append input with multiple text inputs without text input end")
        print("   - Verify strict event sequence order")
        print("   - Verify dump file generation")
        print("=" * 80)
        self.session_id: str = session_id
        self.dump_file_name = f"tts_append_input_without_text_input_end_{self.session_id}.pcm"
        ...
        for group_idx, texts in enumerate(self.text_groups):
            ...
            for i, text in enumerate(texts):
                await self._send_tts_text_input(
                    ten_env, text, request_id, metadata
                )
    ...
    async def _send_tts_text_input(
        ...
        ten_env.log_info(f"Sending tts text input: {text} (request_id: {request_id}, text_input_end: False)")
    ...
    async def _check_audio_frame_timeout(self, ten_env: AsyncTenEnvTester) -> None:
        ...
        while self.timeout_check_active:
            ...
            if self.last_audio_frame_time is not None:
                ...
                if elapsed >= self.audio_frame_timeout_seconds and self.audio_start_count == 1:
                    ten_env.log_warn(f"⏱️  No audio frame received for {elapsed:.2f} seconds (timeout: {self.audio_frame_timeout_seconds}s), stopping test")
                    ten_env.stop_test()
                    break
    ...
    async def on_audio_frame(self, ten_env: AsyncTenEnvTester, audio_frame: AudioFrame) -> None:
        ...
        if not self.timeout_check_active:
            ...
            self.audio_frame_timeout_task = asyncio.create_task(self._check_audio_frame_timeout(ten_env))
    ...
    async def on_stop(self, ten_env: AsyncTenEnvTester) -> None:
        ...
        # Stop timeout check
        self.timeout_check_active = False
        if self.audio_frame_timeout_task is not None and not self.audio_frame_timeout_task.done():
...
def test_append_input_without_text_input_end(extension_name: str, config_dir: str) -> None:
