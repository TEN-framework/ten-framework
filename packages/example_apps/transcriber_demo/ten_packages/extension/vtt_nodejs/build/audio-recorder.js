...
export class AudioRecorder {
    ...
    writeFrame(audioFrame) {
        ...
        try {
            ...
            if (this.audioBuffers.length === 0) {
                ...
                console.log(`[AudioRecorder] Audio format: ${this.sampleRate}Hz, ${this.channels}ch, ${this.bitDepth}bit`);
            }
            // Get audio data
            ...
            if (this.audioBuffers.length % 100 === 0) {
                console.log(`[AudioRecorder] Buffered ${this.audioBuffers.length} frames, ${(this.totalBytesWritten / 1024).toFixed(2)}KB, time: ${(this.currentTimestamp / 1000).toFixed(2)}s`);
            }
        }
        catch (error) {
    ...
    async stop() {
        ...
        console.log(`[AudioRecorder] Stopping recording, total frames: ${this.audioBuffers.length}, total bytes: ${this.totalBytesWritten}`);
        try {
            ...
            console.log(`[AudioRecorder] Total samples: ${totalSamples}, duration: ${(totalSamples / this.sampleRate).toFixed(2)}s`);
            ...
            console.log(`[AudioRecorder] WAV file saved: ${this.audioPath}, size: ${(wavBuffer.byteLength / 1024).toFixed(2)}KB`);
            // Clear buffers
            this.audioBuffers = [];
        }
        catch (error) {
     ...
     */
    convertToFloat32(buffer, bitDepth) {
