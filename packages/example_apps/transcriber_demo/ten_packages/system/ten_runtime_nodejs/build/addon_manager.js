...
export class AddonManager {
            ...
            if (fs.existsSync(manifestPath)) {
                try {
                    ...
                    if (manifestJson.type === "extension") {
                        ...
                        const standaloneBuildingAppManifestPath = path.join(standaloneBuildingAppDir, "manifest.json");
    ...
    setRegisterHandler(name, handler) {
    ...
    async loadAllAddons() {
        ...
        for (;;) {
            ...
            if (fs.existsSync(packageJsonFile)) {
                ...
                const modulePath = path.join(extension_folder, entry.name, "build", "index.js");
    ...
    registerSingleAddon(name, registerContext) {
...
export function RegisterAddonAsExtension(name) {
    return (klass) => {
        function registerHandler(registerContext) {
            const addon_instance = new klass();
            ten_addon.ten_nodejs_addon_manager_register_addon_as_extension(name, addon_instance, registerContext);
        }
...
//# sourceMappingURL=addon_manager.js.map