...
function valueTypeToBufferType(valueType) {
    const mapping = {
...
function bufferTypeToValueType(bufferType) {
    const mapping = {
...
function calculateContentSize(value) {
    ...
    switch (valueType) {
        ...
        case ValueType.JSON_STRING: {
            const [data, error] = valueType === ValueType.STRING
                ? value.getString()
                : value.getJsonString();
            if (error) {
...
function serializeContent(value, buffer, pos) {
    const valueType = value.getType();
    switch (valueType) {
        case ValueType.INVALID:
            assert(false, "Invalid value type");
        ...
        default:
            assert(false, `Unsupported value type for serialization: ${ValueType[valueType]}`);
    }
}
// Serialize a Value to a buffer using only Node.js operations.
export function serializeToBuffer(value) {
    ...
    if (finalPos !== totalSize) {
        assert(false, `Buffer size mismatch: expected ${totalSize}, got ${finalPos}`);
    }
    return buffer;
}
// Validate the buffer header and return header info.
function validateBufferHeader(buffer) {
...
function deserializeContent(buffer, pos, valueType) {
    switch (valueType) {
        case ValueType.INVALID:
            assert(false, "Invalid value type");
            break;
        ...
        case ValueType.BYTES: {
            ...
            if (bufLen === 0) {
                ...
                data = uint8Array.buffer.slice(uint8Array.byteOffset, uint8Array.byteOffset + uint8Array.byteLength);
                pos += bufLen;
            }
        ...
        default:
            assert(false, `Unsupported value type for deserialization: ${ValueType[valueType]}`);
    }
}
// Deserialize a Value from buffer.
export function deserializeFromBuffer(buffer) {
...
//# sourceMappingURL=value_buffer.js.map