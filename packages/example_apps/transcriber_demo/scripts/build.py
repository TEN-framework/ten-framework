...
def build_go_project(
    ...
    if project_name == "main app":
        build_tool = project_dir / "ten_packages" / "system" / "ten_runtime_go" / "tools" / "build" / "main.go"
        ...
        try:
            ...
            print_error(f"Failed to build {project_name} (exit code: {e.returncode})")
            return False
    else:
...
def build_nodejs_project(
    ...
    if not has_build_script(npm_path, project_dir):
        print_warning(f"No build script found for {project_name}, skipping build step")
        return True
    ...
    try:
        ...
        print_warning(f"Failed to clean build artifacts for {project_name}: {e}")
...
def main():
    ...
    if not args.skip_nodejs:
        ...
        if not nodejs_projects:
            ...
            for project_dir, project_name in nodejs_projects:
                if not build_nodejs_project(npm_path, project_dir, project_name):
                    success = False
    ...
    if success:
        ...
        print_error("Some projects failed to build. Please check the logs above.")
        return 1
