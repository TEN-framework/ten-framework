...
def install_python_requirements(
    ...
    try:
        ...
        print_error(f"Failed to install dependencies for {package_name} (exit code: {e.returncode})")
        return False
...
def main():
    ...
    if not args.skip_python:
        ...
        if not requirements_files:
            ...
            print_info(f"Found {len(requirements_files)} Python package(s) with requirements.txt")
            print()
            
            for req_file, package_name in requirements_files:
                if not install_python_requirements(python_path, req_file, package_name, args.pip_index):
                    success = False
    ...
    if not args.skip_nodejs:
        ...
        if not package_json_dirs:
            ...
            print_info(f"Found {len(package_json_dirs)} Node.js package(s) with package.json")
            print()
            
            for package_dir, package_name in package_json_dirs:
                if not install_nodejs_dependencies(npm_path, package_dir, package_name, args.npm_registry):
                    success = False
    ...
    if success:
        ...
        print_error("Some dependencies failed to install. Please check the logs above.")
        return 1
